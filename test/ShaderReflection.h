...
#include "device.h" // Replace with your DXCore etc
...
public:
	...
	void init(device& core, unsigned int sizeInBytes, int constantBufferIndex, ShaderStage _shaderStage)
	{
...
{
public:
	void build(device& core, ID3DBlob* shader, std::vector<ConstantBuffer>& buffers, std::map<std::string, int>& textureBindPoints, ShaderStage shaderStage)
	{
